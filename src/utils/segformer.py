    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cls_seg = nn.Conv2d(
            in_channels=self.channels, out_channels=self.num_classes, kernel_size=1
        )
...
class BilinearPADHead_fast_xavier_init(ASPPHead):
    ...
    def interpolate(self, x, x_cat=None, norm=None):
        ...
        coord = (
            self.coord.reshape(1, H, W, 2, f, f)
            .permute(0, 3, 1, 4, 2, 5)
            .reshape(1, 2, H * f, W * f)
        )
        ...
        coord = (
            coord.reshape(B_coord, ch_coord, H, f, W, f)
            .permute(0, 2, 4, 1, 3, 5)
            .reshape(1, B_coord * H * W * ch_coord, f, f)
        )
        output = self.subnetworks_forward(coord, weights, biases, B * H * W)
        output = output.reshape(B, H, W, self.pad_out_channel, f, f).permute(
            0, 3, 1, 4, 2, 5
        )
    ...
    def subnetworks_forward_fast(self, inputs, weights, biases, n_subnets):
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            if i == 0:
                x = self.padconv(
                    x, w, b, cin=2 + num_bases, cout=self.dyn_ch, relu=True
                )
            if i == 1:
                x = self.padconv(x, w, b, cin=self.dyn_ch, cout=self.dyn_ch, relu=True)
            if i == 2:
                x = self.padconv(
                    x, w, b, cin=self.dyn_ch, cout=self.pad_out_channel, relu=False
                )
        return x
    ...
    def padconv(self, input, w, b, cin, cout, relu):
        ...
        x = F.conv2d(x, conv_w, stride=1, padding=0, groups=cout)
        x = x + b
        if relu:
            x = F.relu(x)
        return x
...
def compute_locations_per_level(h, w):
    shifts_x = torch.arange(0, 1, step=1 / w, dtype=torch.float32, device="cuda")
    shifts_y = torch.arange(0, 1, step=1 / h, dtype=torch.float32, device="cuda")
    shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
    locations = torch.stack((shift_x, shift_y), dim=0)
    return locations
...
class SegFormer(nn.Module):
    ...
    def init_weights(
        self, load_imagenet_model: bool = False, imagenet_ckpt_fpath: str = ""
    ) -> None:
        ...
        if os.path.isfile(imagenet_ckpt_fpath):
            print(
                "===========> loading pretrained models {}".format(imagenet_ckpt_fpath)
            )
            ...
            raise RuntimeError(
                "no pretrained models found at {}".format(imagenet_ckpt_fpath)
            )
...
def get_configured_segformer(
    ...
    model = get_seg_model(
        criterion, n_classes, load_imagenet_model, imagenet_ckpt_fpath
    )
    return model
...
if __name__ == "__main__":
    ...
    loss_method = Cross_sim_loss(
        data_index=["universal"],
        data_root="./data",
        ignore_label=255,
        emd_method="wiki_embeddings",
    )
    criterions.append(loss_method)
    model = get_configured_segformer(
        180, criterions, load_imagenet_model, imagenet_ckpt_fpath
    )
