...
def get_parser() -> CfgNode:
    ...
    parser = argparse.ArgumentParser(
        description="Yvan Yin's Semantic Segmentation Model."
    )
    ...
    parser.add_argument(
        ...
        default="image_",
        type=str,
        help="the images folder name except the camera ID",
    )
    parser.add_argument(
        "--img_file_type",
        default="jpeg",
    ...
    parser.add_argument(
        "--save_folder",
        type=str,
        default="ann/semantics",
        help="the folder for saving semantic masks",
    )
    ...
    parser.add_argument(
        ...
        help="new label definitions identified by user",
    )
    parser.add_argument(
        "opts",
...
def single_scale_single_crop_cuda(
    model, image: np.ndarray, h: int, w: int, gt_embs_list, args=None
) -> np.ndarray:
    ...
    prediction_crop = logit_universal[
...
def do_test(args, local_rank):
    ...
    model = get_configured_segformer(
        args.num_model_classes, criterion=None, load_imagenet_model=False
    )
    model.eval()
    ...
    ckpt_filter = {
        k: v for k, v in checkpoint.items() if "criterion.0.criterion.weight" not in k
    }
    model.load_state_dict(ckpt_filter, strict=False)

    if args.user_label:
        if args.new_definitions:
            gt_embs_list = create_embs_from_names(
                args.user_label, args.new_definitions
            ).float()
        else:
...
def test_single(args, imgs_list, local_rank, model, gt_embs_list):
    ...
    for i, rgb_path in tqdm(enumerate(imgs_list)):
        save_path = os.path.join(
            args.root_dir, args.save_folder, os.path.basename(rgb_path)
        )
        ...
        if args.single_scale_single_crop:
            out_logit = single_scale_single_crop_cuda(
                model, image_resized, h, w, gt_embs_list=gt_embs_list, args=args
            )
        elif args.single_scale_multi_crop:
            out_logit = single_scale_cuda(
                model, image_resized, h, w, gt_embs_list=gt_embs_list, args=args
            )
